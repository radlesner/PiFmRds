...
int fm_mpx_open(char *filename, size_t len)
{
    ...
    if (filename != NULL)
    {
        ...
        {
            low_pass_fir[FIR_HALF_SIZE - 1 - i] = sin(2 * PI * cutoff_freq * i / 228000) / (PI * i) // sinc
...
int fm_mpx_get_samples(float *mpx_buffer)
{
    ...
    if (inf == NULL)
        return 0; // if there is no audio, stop here
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        */
        ...
        mpx_buffer[i] = mpx_buffer[i] +  // RDS data samples are currently in mpx_buffer
                        4.05 * out_mono; // Unmodulated monophonic (or stereo-sum) signal

        if (channels > 1)
        {
            mpx_buffer[i] += 4.05 * carrier_38[phase_38] * out_stereo + // Stereo difference signal
